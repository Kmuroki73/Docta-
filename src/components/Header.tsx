import { Star, Bell, Search, Menu, X, LogOut, ChevronDown } from 'lucide-react';
    ...
    <header className="h-12 sm:h-14 bg-slate-900/95 backdrop-blur-xl border-b border-white/5 flex items-center px-2 sm:px-4 gap-2 z-30 relative flex-shrink-0">
      ...
      {showSearch && (
        <div className="md:hidden absolute inset-0 bg-slate-900 flex items-center px-3 gap-2 z-10">
          ...
          />
          <button onClick={() => { setShowSearch(false); setSearchValue(''); }}
      ...
      <div className="flex items-center gap-0.5 ml-auto">
        <button onClick={() => setShowSearch(true)} className="md:hidden w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors">
          <Search className="w-4 h-4" />
        </button>

        {/* Notifications */}
        <div className="relative">
          <button onClick={() => { setShowNotifications(!showNotifications); setShowUser(false); }}
          ...
          {showNotifications && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-72 bg-slate-800 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="hidden sm:block text-left">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">Free</p>
            </div>
          ...
          {showUser && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">

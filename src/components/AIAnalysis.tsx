...
import {
  ...
  Video, Music, Image as ImageIcon, MessageSquare, StickyNote
} from 'lucide-react';
...
export default function AIAnalysis({ file, analysis, onAnalyze, onFiles, onViewNotes, onViewChat }: AIAnalysisProps) {
        ...
        {activeTab === 'insights' && (
          <>
            ...
            <div className="flex gap-2 mt-4">
              {onViewNotes && (
                <button onClick={onViewNotes} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-blue-600/15 hover:bg-blue-600/25 border border-blue-500/25 rounded-2xl text-blue-400 text-xs font-medium transition-colors active:scale-[0.98]">
                  <StickyNote className="w-3.5 h-3.5" />
                  View Notes
                </button>
              )}
              {onViewChat && (
                <button onClick={onViewChat} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-cyan-600/15 hover:bg-cyan-600/25 border border-cyan-500/25 rounded-2xl text-cyan-400 text-xs font-medium transition-colors active:scale-[0.98]">
